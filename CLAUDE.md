...
| `tool.unsupported` | setup unknown-tool loop + `--request` + `jarvy diagnose <name>` + `jarvy doctor --tools <name>` + `jarvy search <name>` (zero results) | Uniform field shape across all call sites. `source` = `"config"` (setup loop) \| `"request"` (`tools --request`) \| `"diagnose"` \| `"doctor"` \| `"search"`. `diagnose`, `doctor`, and `search` sources are gated behind the telemetry consent gate; `request` bypasses it (explicit user action). `search` only fires when the query is non-empty and returns zero results — fuzzy partial matches do not trigger it. The registered-but-no-platform-installer sites additionally carry `fallback_declared` (bool) + `fallback_blocked` (`"toolchain_uninstallable"` \| `"none_declared"`) — fires only after all declared fallback routes are exhausted (PRD-060). |
