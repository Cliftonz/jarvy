...
| `dotfiles.phase_started` / `dotfiles.phase_completed` / `dotfiles.phase_failed` | `[dotfiles]` block phase lifecycle in setup | started: `manager`, `apply`. completed: `manager`, `duration_ms`. failed: `manager`, `duration_ms`, `error`. Gated. |
...
| `dotfiles.remote_refused` | trust gate refused a remote-origin config's `[dotfiles]` without `allow_remote` | `reason = "allow_remote_not_set"`. Warn. Gated. |
