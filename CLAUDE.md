...
| `agent_profile.created` / `agent_profile.deleted` | `jarvy agents profile init/create/delete` (PRD-058) | `agent_count` only — profile NAMES never emitted. Info. Gated. |
| `agent_profile.switched` | `use` — one event per mechanism so the label stays bounded | `agents` (comma-joined slugs), `mechanism = "env" \| "symlink"`, `scope = "tty" \| "global"`, `duration_ms`. Emitted by the CLI handler ONLY — `switcher::apply_symlink_repoint` is silent (init link-back must not false-fire). Info. Gated. |
