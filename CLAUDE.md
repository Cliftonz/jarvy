...
| `dotfiles.phase_started` / `dotfiles.phase_completed` / `dotfiles.phase_failed` | `[dotfiles]` block phase lifecycle in setup | started: `manager`, `apply`. completed: `manager`, `duration_ms`. failed: `manager`, `duration_ms`, `error_kind = "auth" \| "network" \| "not_found" \| "conflict" \| "binary_missing" \| "permission_denied" \| "spawn_failed" \| "other"`. **NEVER emit raw subprocess stderr as an event field** — git/chezmoi echo the repo URL (with any embedded token) on auth failures. `PhaseOutcome::Failed` carries `error_kind` (bounded, telemetry-safe) and `error` (human message; caller-only). Gated. |
| `dotfiles.phase_skipped` | trust-gate-clean skip paths — dry-run, empty repo, stow (manual-only), or the manager CLI missing from PATH | `reason = "dry_run" \| "empty_repo" \| "stow_manual" \| "manager_not_installed"`. Info. Gated. |
| `dotfiles.remote_refused` | trust gate refused a remote-origin config's `[dotfiles]` without `allow_remote`, OR input-safety refused a leading-`-` / NUL-bearing `repo` (CVE-2017-1000117-class argv-injection into git via `--upload-pack=/tmp/pwn`) | `reason = "allow_remote_not_set" \| "invalid_repo"`. Warn. Gated. Emitted by `emit_refused`; `apply_chezmoi` / `apply_yadm` also scrub GIT_*/CHEZMOI_* code-execution env vars via `dotfile_command` before spawning. |
...
| `services.daemon_check` | container-runtime backend preflight (Docker Compose + Podman Compose) — probes `docker info` / `podman info` before `compose up` | `backend = "docker" \| "podman"`, `state = Running \| Down \| Missing \| Timeout`, `duration_ms`. Debug level. Gated. |
| `services.daemon_down` | preflight returned Down/Missing/Timeout — daemon binary present but socket not reachable (Docker Desktop stopped, colima not started, podman machine down, systemd unit inactive, hung apiserver) | `backend`, `state = Down \| Missing \| Timeout`, `duration_ms`. Warn level. Gated. User sees actionable hint via `ServiceError::DaemonNotRunning`; the event is the OTLP breadcrumb. |
