...
| `services.daemon_down` | preflight returned Down/Missing/Timeout — daemon binary present but socket not reachable | `backend`, `state = Down \| Missing \| Timeout`, `hint_kind = "colima" \| "docker_desktop_macos" \| "docker_desktop_windows" \| "systemd" \| "podman_machine" \| "podman_socket"` (bounded — answers "colima vs Docker Desktop, which suggestion do users see most?"), `duration_ms`. Warn level. Gated. |
...
| `agent_profile.created` / `agent_profile.deleted` | `jarvy agents profile init/create/delete` (PRD-058) | created: `action = "init" \| "create"`, `seeded_from_existing` (bool — `create --from`), `duration_ms`, `agent_count`. deleted: `agent_count`. Profile NAMES never emitted. Info. Gated. |
| `agent_profile.switched` | `use` — one event PER AGENT so `mechanism` stays bounded and per-agent attribution survives | `agent` (slug), `mechanism = "env" \| "symlink"`, `scope = "tty" \| "global"` (symlink rows always `"global"`), `duration_ms`, `invocation_source = "jp_shorthand" \| "cli"` (from `JARVY_JP_INVOCATION`, set by the shell `jp` functions), `first_switch` (bool — agent had no prior `registry.active` entry). Emitted by the CLI handler ONLY — `switcher::apply_symlink_repoint` is silent (init link-back must not false-fire). Info. Gated. |
| `agent_profile.op_failed` | any profile CLI action returned `ProfileError` (exit CONFIG_ERROR) | `error_kind` (bounded `ProfileError` variant names, e.g. `"io"`, `"unmanaged_dir"`, `"profile_not_found"`, `"active_profile_delete"`). Warn. Gated. |
...
| `agent_profile.use_unsupported` | `use --agents` explicitly named a storage-only agent (windsurf/cline/continue, not switchable until v1.1) | `agent`. Explicit-only — an unfiltered `use` skips silently. Info. Gated. |
