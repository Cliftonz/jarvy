...
- `src/git/` — git config automation (identity, signing, aliases). `ConfigValue` = plain string or `{ env, default }`. Signing format auto-detected (`.pub` → ssh, else gpg). Credential helper defaults per-OS. `[git.extra]` is a free-form escape hatch (`HashMap<key,value>`) for un-modeled keys, applied last so they override typed fields. **Remote-config trust gate** (`run_git_phase`): a `ConfigOrigin::Remote` config cannot apply `[git]` unless `[git] allow_remote = true`, and even then writes are forced to `--local` scope (mirrors `[git_hooks] allow_remote`; without it a remote could write `~/.gitconfig`). `[git.extra]` gauntlet (all in `setup.rs`): `validate_extra_key` (dotted grammar + flag-injection, ≤256B) → leading-`-` value refusal (argv option-injection) → `is_exec_capable_key` refuses keys whose value git *executes* (`core.pager`/`sshCommand`/`hooksPath`/`fsmonitor`(non-bool)/`filter.*.clean`/`*.textconv`/… — RCE the `!` filter misses) unless `JARVY_ALLOW_GIT_EXEC_KEYS=1` (`exec_key_value_is_safe` exempts `core.fsmonitor=true|false`) → `check_not_protect_downgrade` (protectNTFS/HFS=false, `safe.directory=*`/`safe.bareRepository=all`, `fsck.*`/`fetch.fsck.*`/`receive.fsck.*=ignore`, `*.fsckObjects=false`; override `JARVY_ALLOW_GIT_PROTECT_DOWNGRADE=1`) → `value_is_shell_escape` (`!` after trim). `os_defaults` (default on; `= false` opts out) applies `os_default_plan()` (pure, unit-testable) for unset keys — `core.autocrlf` (Win `true`/else `input`), Win `core.longpaths`, macOS `core.precomposeunicode`, plus recommended `fetch.prune`/`rerere.enabled`/`merge.conflictStyle=zdiff3`; skips keys already matching (one `existing_config()` read, no redundant forks). All env opt-ins parse via `env_flag_enabled`. See `docs/git-config.md`.
...
| `git_config.phase_started` / `git_config.phase_completed` | setup `run_git_phase` | started: `remote`, `scope`. completed: `ok`, `remote`, `signing`, `aliases`, `extra_key_count`, `os_defaults_enabled`, `duration_ms`. Gated. |
| `git_config.phase_previewed` | dry-run git phase | `os_defaults_enabled`, `extra_key_count`. Gated. |
| `git_config.remote_refused` | remote-origin config applied `[git]` without `allow_remote` | `reason = "allow_remote_not_set"`. Gated. Warn. |
| `git_config.extra_applied` | `[git.extra]` keys written | `key_count`, `sections` (comma-joined DISTINCT section prefixes — never values/full keys). Gated. |
| `git_config.os_defaults_applied` | `configure_os_defaults` (both branches — opt-out graphable) | `enabled`, `opted_out`, `keys_written`. Gated. |
| `git_config.os_default_applied` | per default key, AFTER its write succeeds | `key`, `value` (both jarvy constants). Debug. |
| `git_config.exec_key_refused` | `[git.extra]` key whose value git executes | `key` (never value). Warn. Security refusal — NOT gated (local audit). |
| `git_config.protect_downgrade_refused` | `[git.extra]` value weakens a guardrail | `key`, `guardrail = "protect_ntfs_hfs" \| "safe_directory_wildcard" \| "safe_bare_repository" \| "fsck_objects_disabled" \| "fsck_ignore"`. Warn. Not gated. |
| `git_config.shell_escape_refused` / `git_config.shell_alias_refused` | `!`-shell value / alias refused | `key` / `alias`. Warn. Not gated. |
| `git_config.refused_invalid_key` / `git_config.refused_option_value` | `[git.extra]` key grammar / leading-`-` value refused | `key`. Warn. Not gated. |
| `git_config.set_failed` | `git config` write exited non-zero | `key`, `error` (git stderr, trimmed). Warn. Not gated. |
...
**Telemetry gate.** Every `library.*`, `library.git.*`, `library.git_skill.*`, `skills.*`, `git_hooks.*`, `package.*`, `discover.*`, `workspace.*`, `wizard.*`, and `mcp.mutation.*` / `mcp.auto_approve.*` event reads `observability::telemetry_gate::is_enabled()` before emitting. Users with `telemetry.enabled = false` don't ship event breadcrumbs even when the OTLP exporter is otherwise configured. Review item 7 (P0) — previously the new `library.git.*` / `skills.*` / `git_hooks.*` domains bypassed the gate; now consistent with `packages.*`. The `mcp.auto_approve.*` domain was added to this list in the follow-up review's Obs F11 fix. **`git_config.*` is deliberately split** (parallel-review Obs F6): the adoption/lifecycle events (`phase_started`/`phase_completed`/`phase_previewed`/`extra_applied`/`os_defaults_applied`/`remote_refused`) are gated like the rest, but the **security-refusal** events (`exec_key_refused`, `protect_downgrade_refused`, `shell_escape_refused`, `shell_alias_refused`, `refused_invalid_key`, `refused_option_value`, `set_failed`) are intentionally **NOT** gated — they're local-audit `warn!`s that must survive `telemetry.enabled = false`.
