...
mod tests {
    ...
    #[test]
    fn fd_registration_shape() {
        assert_eq!(FD.command, "fd");
        let mac = FD.macos.expect("must support macOS");
        assert_eq!(mac.brew, Some("fd"));
        let win = FD.windows.expect("must support Windows");
        assert_eq!(win.winget, Some("sharkdp.fd"));
    }
}
