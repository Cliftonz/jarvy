...
/// Bounded sibling to the `platform` field on telemetry events — "wsl" vs
/// "native" distinguishes a WSL distro from bare-metal Linux, a distinction
/// `env::consts::OS` (compile-time) cannot make. Additive: does not change
/// `platform`'s existing value, so existing dashboards/queries keep working.
...
/// Record that a hook script needed CRLF/CR normalization before execution
/// (a script saved with Windows line endings would otherwise corrupt POSIX
/// shell parsing). Signal only — no new failure mode, just adoption-of-fix
/// visibility.
pub fn hook_crlf_normalized(hook_name: &str, hook_type: &str) {
    if !is_enabled() {
        return;
    }
    tracing::info!(
        event = "hook.crlf_normalized",
        hook_name = %hook_name,
        hook_type = %hook_type,
    );
}
